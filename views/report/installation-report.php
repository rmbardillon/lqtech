...
// Row 1
if(isset($_GET['from'])){
    $pdf->Cell(0,6,strtoupper('INSTALLATIONS FOR '.$fromDate." TO ".$toDate),0,1,'C');
} else {
    $pdf->Cell(0,6,strtoupper('INSTALLATIONS FOR FROMDATE TO TODATE'),0,1,'C');
}
...
$pdf->Cell(50,10,'Project Site',1,0,'C');
$pdf->Cell(40,10,'Installer',1,0,'C');
$pdf->Cell(40,10,'S/O',1,0,'C');
$pdf->Cell(40,10,'J/O',1,1,'C');
...
if(isset($_GET['from'])){
    ...
    foreach($result as $key => $value)
    {
        ...
        $pdf->Cell(60,7,$value['PROJECT_NAME'],1,0,'C');
        $pdf->Cell(50,7,$value['CONTACT_PERSON'],1,0,'C');
        $pdf->Cell(40,7,$value['CONTACT_NUMBER'],1,0,'C');
        $pdf->Cell(50,7,$value['PROJECT_SITE'],1,0,'C');
        $pdf->Cell(40,7,$value['INSTALLER'],1,0,'C');
        $pdf->Cell(40,7,$value['SALES_ORDER_NUMBER'],1,0,'C');
        $pdf->Cell(40,7,$value['JOB_ORDER_NUMBER'],1,1,'C');
        $i++;
    }
} else {
    for ($i = 1; $i <= 10; $i++)
